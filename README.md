# 迷路ジェネレーター

迷路を作るところと、解くところをそのまま眺められる WEB アプリ。Vite + Svelte 5 (runes) + TypeScript。

**公開先: https://www.buko106.tokyo/cc-maze/**（`main` への push で GitHub Pages へ自動デプロイ）

- **生成**: 再帰的バックトラッカー / Kruskal 法 / Prim 法 / 再帰分割 / Wilson 法 / Eller 法
- **探索**: 深さ優先探索 (DFS) / 幅優先探索 (BFS) / A\* / 双方向 BFS / 右手法（壁伝い） / 行き止まり埋め
- **自分で解く**: 迷路を指やマウスでなぞって、S から G を目指す
- **配置**: スタートとゴールを左上・右下に固定するか、生成のたびにランダムな 2 セルへ置く
- **ループ**: 生成後に行き止まりを開けて輪を作る（0〜100%）。完全迷路でなくなると手法ごとに違う経路を返す
- **保存**: 選んだ設定はブラウザに残り、次に開いたときも同じ設定から始まる

生成が終わると探索ボタンが有効になる。生成と探索はそれぞれ独立に手法と速度を選べるので、
同じ迷路を別の手法で解き直して見比べられる。「自分で解く」も探索の選択肢の 1 つとして並んでおり、
自分でなぞった結果をアルゴリズムと同じ「調べたセル」で比べられる。

```bash
npm install
npm run dev
```

| コマンド         | 内容                                         |
| ---------------- | -------------------------------------------- |
| `npm run dev`    | 開発サーバー (http://localhost:5173)         |
| `npm run build`  | 本番ビルド → `dist/`                         |
| `npm run check`  | svelte-check + tsc による型チェック          |
| `npm test`       | Vitest（`test:watch` で監視）                |
| `npm run lint`   | ESLint（`lint:fix` で自動修正）              |
| `npm run format` | Prettier で整形（`format:check` で検査のみ） |

## 開発ツール

| ツール                            | 担当                                                                                         |
| --------------------------------- | -------------------------------------------------------------------------------------------- |
| Vitest + fast-check               | テスト。`vite.config.ts` を共有するので設定が二重にならない                                  |
| Prettier + prettier-plugin-svelte | 整形。`.svelte` はマークアップ・script・style すべて                                         |
| ESLint + eslint-plugin-svelte     | lint。テンプレートまで構文解析するので、マークアップでしか使わない変数を未使用と誤検出しない |
| svelte-check                      | 型・a11y・未使用 CSS の検査                                                                  |
| Lefthook                          | Git フック。pre-commit で Prettier → ESLint、commit-msg で commitlint                        |
| commitlint                        | Conventional Commits の検証                                                                  |
| GitHub Actions                    | CI。全ブランチと PR で型チェック・lint・テスト・ビルド。`main` だけ続けて Pages へデプロイ   |

Biome も検討したが、`.svelte` は `<script>` ブロックしか読めない。マークアップが見えないため
テンプレートでしか使わない変数がすべて未使用として誤検出され、フォーマッタは script ブロックの
インデントを剥がして Svelte 標準の整形と衝突する。このプロジェクトは `.svelte` が中心なので、
Svelte 公式アドオン（`sv add prettier eslint`）と同じ構成に一本化した。

`npm install` すると `prepare` スクリプトが `lefthook install` を走らせ、フックが有効になる。

### テスト

```bash
npm test            # 1 回だけ実行
npm run test:watch
FC_SEED=$RANDOM npm test   # 固定シード以外の入力も試す
```

テストランナーは **Vitest**。Vite プロジェクトなので `vite.config.ts` をそのまま共有でき、変換や
エイリアスの設定が二重にならない（Svelte 公式も Vite を使うなら Vitest を挙げている）。対象は
`src/lib/maze/` の純粋な TypeScript なので `environment` は `node` で、DOM も jsdom も要らない。
`src/lib/settings/` も同じで、`localStorage` そのものではなく `getItem` / `setItem` だけを持つ
差し替え可能な入れ物を読み書きするので、node のままで「保存できない環境」まで試せる。

#### 乱数が絡むテストの書き方

迷路は乱数で形が変わる。「この入力ならこの出力」を書き下せないので、3 つの原則で組んでいる。

**1. 乱数は必ず注入する。** 生成アルゴリズムは `rng: () => number` を引数で受け取り、`Math.random` を
直接呼ばない。テストは `createRng(seed)` を渡すだけで完全に決定的になる。この約束自体も、
`Math.random` を呼んだら落ちるようにしたテストで検査している。

**2. 値ではなく不変条件を書く。** どんな迷路になったかではなく、どんな迷路でも成り立つ性質を検査する。

| 対象   | 不変条件                                                                    |
| ------ | --------------------------------------------------------------------------- |
| 生成   | 全セル連結・辺の数 = セル数 − 1・リンクは両側に立つ（つまり完全迷路）       |
| 配置   | S と G は重ならない・行ける最遠距離の半分以上離れる・外壁を開けるのは端だけ |
| ループ | 壁は開くだけで閉じない・連結は保たれる・1 ステップ = 1 枚の壁               |
| 探索   | 経路が壁を越えない・S に始まり G に終わる・完全迷路ならどの手法も同じ経路   |
| なぞる | どう操作しても線は壁を越えず、同じセルを二度通らない・調べたセル = 塗った数 |
| 設定   | 保存されていた中身が何であれ、全項目がその項目の検証を通る値で返る          |

**3. 入力は fast-check に振らせ、シードは固定する。** サイズ・シード・ループ率を property-based
testing で振りつつ、`vitest.setup.ts` で `fc.configureGlobal({ seed })` を固定する。CI でも手元でも
同じ 30 通りが回るので、赤は赤のままになる。別の入力を試したいときは `FC_SEED` を渡す。失敗すると
シードと**縮小された反例**（「5×5・シード 12345 で落ちる」など）が出るので、それを回帰テストに残せる。

止まらなくなりうる箇所には**ステップ上限**を置いた。ヘルパの `drain` は一定ステップを超えたら例外を
投げる。ハングした CI は失敗した CI よりずっと読み解きにくい。上限は実測に基づく（20×20 の 300
シードで、生成の最大は Wilson の 12,942 ステップ、探索の最大は 682 ステップ）。

ただし上限が効くのは `yield` するコードだけで、行き止まり埋めの経路たどりのように最後の `yield`
より後ろにある処理は、そこが無限ループになるとテストごと固まる。そこは「返す経路が同じセルを
二度通らない」という結果側の不変条件で押さえている。

テストがバグを捕まえられるかは、コードをわざと壊して確かめた（ミューテーションテスト）。壁伝いの
循環検出を外す・ループ処理で片側の壁しか開けない・mulberry32 の定数を変える・A\* の重複展開を許す、
などをそれぞれ入れて、すべてテストが落ちることを確認している。「自分で解く」も同じで、壁の判定を
外す・線を切り戻さない・ゴールで止めない・飛ばしたマスを補わない、など 10 通りを入れてすべて落ちた。

### 言語の使い分け

**コード内コメントとコミットメッセージは英語で書く。**

| 対象                                                       | 言語   |
| ---------------------------------------------------------- | ------ |
| コード内コメント（`.ts` / `.svelte` のコメント、JSDoc）    | 英語   |
| コミットメッセージ（type / scope / subject / body すべて） | 英語   |
| UI に表示される文字列（アルゴリズム名・ラベル・凡例など）  | 日本語 |
| ドキュメント（README.md / CLAUDE.md）                      | 日本語 |

### コミットメッセージ

Conventional Commits に従う。commit-msg フックで検証される。

```
feat(maze): draw start and goal markers
fix(renderer): stop outer walls from being clipped in half
chore: set up Prettier, ESLint and Lefthook
```

## 実装の考え方

アルゴリズムを **ジェネレータ関数** として書くのが中心的な設計判断。生成も探索も同じ形をしている。

```ts
export type MazeAlgorithm = (ctx: MazeContext, rng: () => number) => Generator<void, void, void>
export type SolveAlgorithm = (ctx: SolveContext) => Generator<void, void, void>
```

各アルゴリズムはコンテキスト（盤面 + 各セルの進捗状態）を破壊的に更新しながら 1 ステップずつ `yield` する。呼び出し側は

- 毎フレーム少しずつ `next()` を呼べば **過程のアニメーション**
- `done` になるまで回し切れば **一括実行**

を同じコードから得られる。アルゴリズム側にアニメーション用の分岐は一切要らない。

探索側に `rng` を渡していないのは意図的で、同じ迷路を同じ手法で解けば必ず同じ結果になる。手法どうしの比較が再現できる。

迷路は「セルごとに、どの方向へ通路が開いているか」のビットマスク (`Uint8Array`) で保持する。壁のリストを持つより軽く、隣接セルとビットを一組で立てるだけで整合が取れる。

ループ（`braid.ts`）は生成アルゴリズムと同じ形のジェネレータなので、`yield*` で生成のうしろに繋ぐだけでよい。
呼び出し側からは 1 本のジェネレータに見え、アニメーションのループは 2 段階で作られていることを知らずに済む。

```ts
function* buildMaze(ctx: MazeContext, rng: () => number) {
  yield* algorithm.run(ctx, rng)
  yield* braid(ctx, rng, settings.braidPercent / 100)
}
```

スマホ縦持ちでは、迷路を画面上部に `position: sticky` で貼り付け、生成・探索のボタンを下端へ固定する。
アルゴリズム 12 個ぶんのカードと凡例で設定は縦に長くなるが、迷路と操作ボタンを見失わずにスクロールできる。
初期サイズも画面幅で切り替える（狭ければ 18×18）。28×20 のままだとセルが 10px 前後になり、探索の色分けが潰れるため。
「自分で解く」を選んでいる間は迷路に `touch-action: none` を付け、迷路の上のスワイプはスクロールではなく
線を引く操作になる。スクロールは迷路より下の設定部分で行う。

ボタンやラベルには `touch-action: manipulation` を指定する。生成ボタンと解くボタンは近いので、続けて押すと
ダブルタップと判定されて画面が拡大されてしまう。この指定でダブルタップ拡大だけが外れ、ピンチ操作による拡大と
スクロールはそのまま残る。

生成の進捗 (`MazeContext.state`) と探索の進捗 (`SolveContext.state`) は別々の配列。同じ画面に重ねて描いても、色の意味が混ざらない。

描画は Canvas 2D。差分描画はせず毎フレーム全部描き直すが、同じ色のセルをまとめて塗り、壁は 1 本の Path に集約して一度だけ `stroke()` するので、6,300 セル（90×70）でも 1 フレームに収まる。経路はセル中心を結ぶ 1 本の太いラインとして、壁より先に描く。

### 生成手法の見どころ

同じ完全迷路でも、作り方によって見た目の癖がはっきり違う。3 つは通路を掘り、1 つは壁を立て、
2 つはそれ以外の理屈で組み上げる。

| 手法                   | 癖                                                             |
| ---------------------- | -------------------------------------------------------------- |
| 再帰的バックトラッカー | 長い一本道と少ない分岐。解きごたえは一番                       |
| Kruskal 法             | 均質で、どこを見ても同じような密度                             |
| Prim 法                | 短い行き止まりだらけ。1 点から放射状に広がる                   |
| 再帰分割               | 唯一の壁を立てる方式。一直線の長い壁と部屋ができる             |
| Wilson 法              | 形の偏りが一切ない（一様ランダム）。そのぶん生成は桁違いに遅い |
| Eller 法               | 1 行ずつ確定。縦に長い区間が出やすい                           |

生成ステップ数の桁の違いがそのまま速さの違いになる。28×20 で、再帰的バックトラッカー 1,119 /
Eller 249 に対し、Wilson は 2,866 ステップかかる。

### 探索手法の見どころ

完全迷路では 2 点を結ぶ道は 1 本しかないので、**どの手法でも見つかる経路は同じ**になる。差が出るのは「そこへ行き着くまでに何セル調べたか」で、パネルの `調べたセル` がそれを表す。

28×20（560 セル）の迷路を 6 種の生成手法 × 20 シード = 120 個作って平均したもの。スタートとゴールは
角に固定（下の「スタートとゴールを動かすと」を参照）。経路長の平均は 89:

| 手法           | 調べたセル（平均） | 割合 |
| -------------- | ------------------ | ---- |
| 右手法         | 304                | 54%  |
| DFS            | 312                | 56%  |
| 双方向 BFS     | 330                | 59%  |
| A\*            | 343                | 61%  |
| BFS            | 470                | 84%  |
| 行き止まり埋め | 471                | 84%  |

双方向 BFS が BFS より安いのは、波が半分の距離で済むから。ただし差は迷路の形に強く依存し、
長い一本道ばかりの再帰的バックトラッカーの迷路ではほぼ互角になる（367 対 370）。

行き止まり埋めだけは性質が違う。経路を「探す」のではなく経路以外を「消す」ので、ゴールの方向を
まったく見ず、常に経路以外のほぼ全セルを塗ることになる。`調べたセル + 経路の長さ` が必ず全セル数
に一致する。

### ループを入れると

`ループ` スライダーは、掘り終えた迷路の行き止まりの壁を開けて輪を作る（braiding）。0% なら完全迷路の
まま、100% なら行き止まりがひとつも残らない。行き止まりどうしを繋げられるときはそちらを選ぶので、
実際の行き止まりは指定した割合よりやや速く減る（28×20 の再帰的バックトラッカーで 54 → 26 → 0）。

輪ができた時点で「2 点を結ぶ道は 1 本」という前提が崩れ、**手法ごとに返す経路そのものが変わる**。
50% のループを入れた 20 個の迷路では、BFS と DFS が別の長さの経路を返したものが 18 個あった。
BFS・A\*・双方向 BFS は最短を返し続けるが、DFS と右手法は先に行き当たった道を返す。

前提を失う手法が 2 つある。UI では選んだときに警告を出す。

| 手法           | ループがあると                                                                                             |
| -------------- | ---------------------------------------------------------------------------------------------------------- |
| 右手法         | 壁が島になると回り続けうる。S と G が角なら外壁伝いに必ず着くが、通った輪が経路に残る                      |
| 行き止まり埋め | ループには埋める行き止まりがなく、残った分かれ道を選べない。少しでもループがあるとほぼ「到達できず」になる |

どちらも止まらなくなりうるので歯止めを入れてある。右手法は「セルと向き」の組が一度出たら打ち切り
（同じ状態に戻った時点で以降は繰り返しにしかならない）、行き止まり埋めは経路をたどる際に一度通った
セルへ戻らない。生成 6 種 × 3 サイズ × ループ率 6 段階 × 8 シードの 864 通りで、両者とも必ず停止し、
行き止まり埋めは 809 通りで「到達できず」を返した。

### スタートとゴールを動かすと

`スタートとゴール` は 2 通り。**固定**は左上と右下の角で、外壁の上と下に出入口が開く。**ランダム**は
生成のたびに 2 セルを選び直す。スタートを一様に選び、ゴールは「そこから行けるいちばん遠い距離」の
半分以上離れたセルから選ぶので、隣り合ったり互いのすぐ裏に出たりはしない。選ばれたセルが外壁に
接していれば、そのセルが面している外壁を 1 面だけ開ける。内側に落ちたセルには出入口がなく、S と G
は色を塗っただけのセルになる。

配置は掘る前に決まるだけで、生成アルゴリズムは一切見ない。完全迷路では 2 点を結ぶ道は 1 本なので、
どこに置いても「どの手法でも同じ経路」は変わらない。変わるのは長さで、28×20 の平均は 89 → 65 と
短くなる（角どうしは、その迷路で最も遠い組み合わせにあたるため）。

前提が変わるのは右手法だけ。完全迷路なら壁はすべて外壁と繋がった一枚の島なので、どこから始めても
必ずゴールを通る。ループを入れると外壁から切り離された島ができ、そこに乗ると回り続ける。角固定なら
外壁を伝ったまま反対の角へ着くので 1,620 通りで一度も失敗しなかったが、ランダム配置では 1,074 通り
（66%）が「到達できず」に終わった。行き止まり埋めは元から配置に関係なく、76% が 80% になった程度。

### 自分で解く

人がなぞった線も、探索と同じ `SolveContext` に書き込む。線は `path` そのもので、一度でも線が
通ったセルは、線を引き戻したあとも `SEARCHED` のまま残る。つまり人の試行は DFS とまったく同じ
形のデータになり、描画も「調べたセル」の数え方も探索のものがそのまま使える。

数え方が本当に同じであることはテストで押さえてある。DFS と同じ順になぞれば `dfs.ts` と、右手を
壁につけたままなぞれば（完全迷路の）右手法と、経路も調べたセルもステップ数も一致する。

操作の決まりは `trace.ts` にまとめてある。

- 線は通路を 1 マスずつしか進まず、壁の向こうへは伸びない。来た道へ戻ると縮む
- 線の途中を押すと、そこから先を消して描き直せる（1 ステップと数える）
- ポインターの位置が線の先から 2 マス離れていても、間のマスが通路なら補って進む。速くなぞって
  マスを飛ばしたときや、角を斜めに切ったときのため。それより遠いときは、線は止まったまま待つ
- G に着いたら線は確定し、「解き直す」か「クリア」までは動かない

ループがあると、輪を回って線の途中に戻ってくることがある。そのときは戻ってきたセルまで線を
切り戻すので、線はいつでも同じセルを二度通らない 1 本の経路になる。G に着けばどの道でも「到達」で、
最短である必要はない。BFS の経路の長さと比べれば、どれだけ遠回りしたかが分かる。

ポインターの位置からセルへの換算は、描画と同じ `measure()`（`renderer.ts`）を通すので、描いた
迷路と判定がずれることはない。`MazeCanvas.svelte` は前回のイベントからの移動を 1/4 セル刻みで
たどり、通過したセルを順に渡す。

`solvers/index.ts` の一覧では「自分で解く」を先頭に置いているが、最初に選ばれているのは DFS の
まま（`DEFAULT_SOLVER_ID`）。初めて開いた人が「解く」を押せば、これまでどおり探索が動き出す。

## 設定の保存

パネルで選んだ設定は `localStorage`（キーは `cc-maze:settings`）に入り、次に開いたときも同じ設定から
始まる。保存するのは生成手法・探索手法・S と G の置き方・サイズ・ループ率・速度 2 つ。迷路そのものや
生成の進捗は保存しない。開くたびに新しい迷路を作る。

設定は増える。**設定を 1 つ足したときに、前のバージョンで保存した値が読めなくなる**のがいちばん
避けたい壊れ方なので、そこを軸に組んである。

**1 か所に宣言する。** `settings/schema.ts` に、設定 1 つにつき 1 行。既定値と「保存された値の読み方」
を持つ。

```ts
export const SETTINGS = {
  algorithmId: choice(ids(algorithms), algorithms[0].id),
  cols: integer(COLS, NARROW ? 18 : 28),
  braidPercent: integer(BRAID, 0),
  // ...
} satisfies SettingsSchema
```

型 `Settings` はこの表から導出されるので、1 行足せば App 側の型もそのぶん増える。スライダーの
`min` / `max` も同じ `COLS` / `ROWS` / `BRAID` を読むため、UI が受け付ける値と保存時に通る値がずれない。
保存も復元もこの一覧を回るだけで、項目名を書いた場所は他にない。

**項目ごとに読む。** 保存された JSON は「信用できない `unknown` の袋」として扱い、項目ごとに検証を
通す。読めない値はその項目だけが既定値に戻り、他は生き残る。

| 保存されていた中身             | 読み込み結果                     |
| ------------------------------ | -------------------------------- |
| 知らないキー                   | 捨てる                           |
| 足したばかりの設定のキーがない | その項目だけ既定値               |
| 消えた手法の id                | その項目だけ既定値、他はそのまま |
| 範囲外のサイズ（`cols: 9999`） | 範囲の端へ丸める（90）           |
| JSON として壊れている・別の形  | 全部既定値                       |

つまり**項目を足す・消す・レジストリから手法を 1 つ外す、のどれもバージョンを上げずに済む**。
`SETTINGS_VERSION` を上げるのは「同じキーの意味が変わった」ときだけで、そのときは保存済みの値を
まとめて捨てる。誰も選んでいない設定を復元するくらいなら初期状態から始める方がよい。

**初回訪問では書かない。** 保存するのは設定を最初に変えたときから。何も触っていない人の
localStorage に既定値を焼き付けると、あとで既定値を変えてもその人には届かなくなる。

読み書きはどちらも `try` で囲ってある。Safari のプライベートモードや Cookie を切った環境では
`localStorage` に触るだけで例外が飛ぶが、設定が残らないだけでアプリはそのまま動く。

### 設定を足す

1. `settings/schema.ts` の `SETTINGS` に 1 行足す
2. `App.svelte` に `settings.<新しい項目>` を読み書きするコントロールを置く

生成・探索の手法を足したときは、id の一覧をレジストリから作っているので `settings/` は触らなくてよい。

## ディレクトリ

```
src/
  App.svelte              UI と再生ループ（rAF）
  lib/
    MazeCanvas.svelte     canvas のサイズ追従と再描画、なぞる操作の受け口
    speeds.ts             再生速度の一覧（レジストリ）
    settings/
      fields.ts           設定 1 項目の型と、検証つきの項目ヘルパ
      schema.ts           保存する設定の一覧（ここに 1 行足すだけ）
      storage.ts          localStorage への読み書き（項目ごとに検証して復元）
      settings.test.ts    テスト
    maze/
      types.ts            Grid / MazeContext / SolveContext と 2 つのアルゴリズム型
      grid.ts             セル・方向・リンク操作、S と G の置き方、探索用の近傍と経路復元
      placements.ts       S と G の置き方の一覧（レジストリ）
      rng.ts              mulberry32（シードを固定すれば同じ迷路になる）
      braid.ts            行き止まりを開けて輪を作る後処理（生成のうしろに繋ぐ）
      trace.ts            自分で解く: なぞった線を SolveContext に書き込む
      *.test.ts           テスト。fast-check で入力を振り、不変条件を検査する
      test-utils.ts       テスト用のヘルパ（完全迷路の判定・経路の検証・ステップ上限）
      renderer.ts         Canvas 2D への描画（生成と探索を重ねて描く）
      algorithms/         生成
        index.ts          UI に出す一覧（レジストリ）
        backtracker.ts    再帰的バックトラッカー
        kruskal.ts        Kruskal 法
        prim.ts           ランダム化 Prim 法
        division.ts       再帰分割（唯一の壁を立てる方式）
        wilson.ts         Wilson 法（ループ消去ランダムウォーク）
        eller.ts          Eller 法（1 行ずつ）
      solvers/            探索
        index.ts          UI に出す一覧（レジストリ）
        dfs.ts            深さ優先探索
        bfs.ts            幅優先探索
        astar.ts          A*（マンハッタン距離 + 二分ヒープ）
        bidirectional.ts  双方向 BFS
        wallfollower.ts   右手法（壁伝い）
        deadend.ts        行き止まり埋め
```

`types.ts` は `MazeContext` / `SolveContext` の両方を持つ。`grid.ts` も同様に、生成用の
`createContext` / `link` と探索用の `createSolveContext` / `openNeighbors` / `tracePath` を並べている。
`cornerEndpoints` / `randomEndpoints` も盤面の座標を扱う話なのでここに置き、`placements.ts` は
`algorithms/index.ts` などと同じく UI に出す一覧だけを持つ。

`settings/` と `speeds.ts` が `maze/` の外にあるのは、迷路の理屈を何も知らないため。速度は 1 フレームに
進めるステップ数の話で、設定はどの選択肢を覚えておくかの話でしかない。逆向きの依存はあり、
`settings/schema.ts` は各レジストリを読んで「保存してよい id」の一覧を作る。

## アルゴリズムを足す

生成なら:

1. `src/lib/maze/algorithms/` に `MazeAlgorithm` を 1 つ書く
2. `algorithms/index.ts` の配列に 1 行足す

探索なら `src/lib/maze/solvers/` に `SolveAlgorithm` を書いて `solvers/index.ts` に 1 行足す。
UI の選択肢と説明文はそこから生成されるので、他に触る場所はない。一覧の中で `run` を持たない
項目が「自分で解く」で、これだけは探索を走らせる代わりに `trace.ts` が `SolveContext` を埋める。
探索を走らせるテストは `run` を持つ項目だけを集めた `searches` を回す。

生成物は **完全迷路**（全セルが連結し、閉路が 1 つもない全域木）であることが前提。新しい生成
アルゴリズムを足したら、辺の数が「セル数 − 1」になっているか、全セルに到達できるかを確認して
おくとよい。探索を足したときは、返す経路がスタートに始まりゴールに終わること、隣り合うセルの
間に壁がないこと、そして BFS で求めた経路と一致することを確かめるとよい。
